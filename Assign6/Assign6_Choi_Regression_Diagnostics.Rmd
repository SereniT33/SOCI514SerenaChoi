...
We will further examine if there are colinearlity among the independent variables. 

```{r}

#inspect correlations
cor(chriot, use="pairwise.complete.obs")	#create bivariate correlation matrix

#install packages "car" and "carData"
library(car)
library(carData)
vif(model2)	# vif gives the variance inflation factor for each variable

# Let's try dropping men's wages
model3<-lm(womwork~fertil+fwage)
summary(model3)

vif(model3)

#Given these results, what would you advise for the analysis of 
...
par(mfrow=c(1,1))
plot(c)
abline(h=(4/(n-k-1)), col="red")
abline(h=-(4/(n-k-1)), col="red")
identify(c, label=occtitle)





Check for collinearity among the independent variables. Do you have any reason to suspect it?

## Addressing influential data and collinearity 

Based on what you found in #2 and #3, how might you address any influential data and collinearity?
